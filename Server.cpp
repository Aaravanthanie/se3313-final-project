...
public:
    GameManager(Socket *p1, Socket *p2 ,int gameNo) : player1(p1), player2(p2), gameNumber(gameNo){}
    
    virtual long ThreadMain() override {
          //game number
        // Game logic here
        // For demonstration, let's assume running the ChessGame as a separate program
        // Replace this with your actual ChessGame invocation
    ...
    try {
        while (true) {
            ...
            if (clientSockets.size() >= 2) {
                gameNum++;
                std::cout << "New game: " << gameNum << std::endl;
                // Create a game session with the first two clients
                GameManager* game = new GameManager(clientSockets[0], clientSockets[1], gameNum);
